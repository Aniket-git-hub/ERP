...
async function createPaymentReceiptService(userId, employeeId, attendanceMonthDate, date, remarks, modeOfPayment, deductionAmount) {
    ...
    try {
        ...
        const attendance = await getAttendanceService(userId, employeeId, firstDay, lastDay)
        const { totalHoursWorked, overtime, holidays: leaves } = await getAggregateAttendanceService(userId, employeeId, firstDay, lastDay)
        ...
        if (deductionAmount && deductionAmount > 0) {
            ...
            for (let i = 0; i < advances.length; i++) {
                ...
                if (currentAdvance.remainingAmount < totalDeductionAmount) {
                    ...
                    await updateAdvanceService(userId, employeeId, currentAdvance.id, {
                        remainingAmount: 0
                    }, { transaction });

                } else if (currentAdvance.remainingAmount > totalDeductionAmount) {
        ...
        const paymentReceipt = await PAYMENT_RECEIPT.create({
            ...
            amount: deductionAmount && deductionAmount > 0 ? totalSalaryAmount - deductionAmount : totalSalaryAmount,
            date,
            salary,
            totalHoursWorked,
